...
def build_firstPart_model(input_var, keep_prob_=0.5):
    ...
    network = tf.layers.max_pooling1d(
        inputs=network, pool_size=8, strides=8, padding="same"
    )
    ...
    # Max pooling
    network = tf.layers.max_pooling1d(
        inputs=network, pool_size=4, strides=4, padding="same"
    )
    ...
    # Max pooling
    network = tf.layers.max_pooling1d(
        inputs=network, pool_size=2, strides=2, padding="same"
    )
...
def build_network(
    hparams, char2numY, inputs, dec_inputs, keep_prob_=0.5,
):
    ...
    if hparams.akara2017 is True:
        ...
        _inputs = tf.reshape(
            inputs, [-1, hparams.n_channels, hparams.input_depth / hparams.n_channels]
        )
    ...
    with tf.variable_scope("encoding") as encoding_scope:
        if not hparams.bidirectional:
            ...
            stacked_cell_fw = tf.contrib.rnn.MultiRNNCell(
                [lstm_cell() for _ in range(hparams.lstm_layers)], state_is_tuple=True
            )
    ...
    with tf.variable_scope("decoding") as decoding_scope:
        ...
        decoder_lengths = np.ones((hparams.batch_size), dtype=np.int32) * (
            hparams.max_time_step + 1
        )
        ...
        if hparams.use_attention:
            ...
            attention_mechanism = tf.contrib.seq2seq.LuongAttention(
                hparams.num_units * 2 if hparams.bidirectional else hparams.num_units,
                encoder_outputs,
        ...
        decoder = tf.contrib.seq2seq.BasicDecoder(
            decoder_cells, training_helper, encoder_state, output_layer=output_layer
        )
    ...
    if not hparams.use_beamsearch_decode:
        ...
        inference_decoder = tf.contrib.seq2seq.BasicDecoder(
            decoder_cells, inference_helper, encoder_state, output_layer=output_layer
        )
    else:
        ...
        decoder_initial_state = decoder_cells.zero_state(
            hparams.batch_size * hparams.beam_width, tf.float32
        ).clone(cell_state=encoder_state)
...
random.seed(
    654
)  # to make have the same training set and test set each time the code is run, we use a fixed random seed
...
def build_whole_model(hparams, char2numY, inputs, targets, dec_inputs, keep_prob_):
    ...
    logits, pred_outputs, dec_states = build_network(
        hparams, char2numY, inputs, dec_inputs, keep_prob_
    )
    ...
    with tf.name_scope("optimization"):
        ...
        beta = 0.001
        lossL2 = (
            tf.add_n([tf.nn.l2_loss(v) for v in vars if "bias" not in v.name]) * beta
        )
        ...
        for i in range(logits.get_shape().as_list()[-1]):
            ...
            loss_is.append(
                tf.contrib.seq2seq.sequence_loss(
                    logits, targets, weights_i, average_across_batch=False
                )
            )
...
def run_program(hparams, FLAGS):
    ...
    for fold_idx in range(num_folds):
        ...
        fname = os.path.join(
            traindata_dir,
            "trainData_" + channel_ename + "_SMOTE_all_10s_f" + str(fold_idx) + ".npz",
        )
        ...
        if os.path.isfile(fname):
            ...
            data_loader.save_to_npz_file(
                X_train, y_train, data_loader.sampling_rate, fname
            )

        X_train = X_train[
            : (X_train.shape[0] // hparams.max_time_step) * hparams.max_time_step, :
        ]
        y_train = y_train[
            : (X_train.shape[0] // hparams.max_time_step) * hparams.max_time_step
        ]
            ...
            if ckpt_exist:
                ...
                for epoch_i in range(hparams.epochs):
                    ...
                    for batch_i, (source_batch, target_batch) in enumerate(
                        ...
                        y_pred_ = batch_logits[:, : hparams.max_time_step].argmax(
                            axis=-1
                        )
                    ...
                    cm = confusion_matrix(
                        y_true, y_pred, labels=range(len(char2numY) - 2)
                    )
                    ...
                    if (epoch_i + 1) % hparams.test_step == 0:
                        ...
                        if (
                            np.nan_to_num(f1_macro) > pre_f1_macro
                        ):  # save the better model based on the f1 score
                            print(
                                "Loss {:.4f} after {} epochs (batch_size={})".format(
                                    loss_track[-1], epoch_i + 1, hparams.batch_size
                                )
                            )
                            ...
                            save_dict = {
                                ...
                                "y_pred": y_pred,
                                "ck_score": ck_score,
                                "alignments_alphas_all": alignments_alphas_all[
                                    :200
...
def main(args=None):
    ...
    tf.app.flags.DEFINE_string(
        "data_dir",
        "data_2013/eeg_fpz_cz",
        """Directory where to load training data_2013.""",
    )
    tf.app.flags.DEFINE_string(
        "output_dir",
        "outputs_2013/outputs_eeg_fpz_cz",
        """Directory where to save trained models """ """and outputs.""",
    )
