...
# TODO: remove returned labels
def load_upload_file(npz_file, seq_len=10):
...
def predict(hparams, FLAGS, filename):
    def get_y_pred(hparams, X_test, classes):
    ...
    X_test, y_test = load_upload_file(npz_file, seq_len=hparams.max_time_step)
    # print "X inputs", X_test
    # preprocessing
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        y_pred = get_y_pred(hparams, X_test, classes)
...
def main(args=None):
    ...
    # filename = "SC4822G0.npz"
    filename = "ST7022J0.npz"
    label_pred = predict(hparams, FLAGS, filename)
    # eeg_frag_score = eeg_frag_info(label_pred)
